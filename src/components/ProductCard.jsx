...
export default function ProductCard({ product }) {
  ...
  return (
    <div className="flex flex-col gap-2 p-1 max-w-1/2 md:max-w-1/3 lg:max-w-1/4 mt-1">
      ...
      <div className="flex flex-col p-4 items-center transition-all ease-in-out duration-300">
        ...
        <button className="font-light text-lg rounded-md bg-primary text-neutral-900 px-2 pt-1 cursor-pointer hover:scale-105 hover:shadow-sm transition-all" onClick={() => addItem(product)}>Add to cart</button>
      </div>
    </div>
  );
}