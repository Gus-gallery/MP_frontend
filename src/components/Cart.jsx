...
export default function Cart({ onClose }) {
  ...
  return (
    <div className="text-left bg-zinc-700 border border-zinc-400 rounded-l-xl absolute top-0 right-0 p-4 flex flex-col gap-4">
          ...
          : items.map(item => (
            <div key={item.id} className="flex flex-col items-center gap-4 aspect-square h-60 text-left">
              ...
              <div>
                ...
                <button className="bg-red-900 rounded-full cursor-pointer hover:scale-105 hover:bg-red-800 transition-all px-1.5 w-fit -mt-4" onClick={() => removeItem(item.id)}>✕</button>
                </div>
              </div>
            </div>
          ))
